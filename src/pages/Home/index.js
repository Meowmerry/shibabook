...
class Home extends Component {
	...
	getFinalPrice() {
		return (this.state.inputPrice - (this.state.inputPrice * this.totalDiscount() / 100)).toFixed(2);
	}
    showPrice =(e)=>{
		this.setState({
				...
				<Row className="home-content" span={24} >
					...
					<Col span={8}>
						...
						<div>
							...
							<h1>{this.state.inputPrice} THB - {this.state.total}% = {this.state.finalPrice} THB</h1>
							<label>
								Price : &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
               					<input name="price" value={this.state.inputPrice} 
								//    onChange={(e)=> this.showPrice(e)} 
								   onChange={(e) => this.setState({inputPrice: e.target.value(e)})}
								    bvcxz type="number" step="0.05" min="0" /> $</label>
							<br />
							<label>
								Discount 1 :
               						<input
